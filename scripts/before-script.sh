...
if [[ ${SHARPEYE_BROWSER} == "chrome" ]]; then
    # Pin chrome.
    docker run -d -p 4444:4444 --shm-size 256m --net=host selenium/standalone-chrome:3.141.59-zinc
elif [[ ${SHARPEYE_BROWSER} == "firefox" ]]; then
    # Use firefox 64 (nearest to Firefox Quatum 64 ESR).
    docker run -d -p 4444:4444 --shm-size 256m --net=host selenium/standalone-firefox:3.141.59-titanium
fi
